var HairdressingTime=function(){
	...
	var batchDelete = function(){
		...
		$.ajax({
			type: "POST",
			url: urls,
			data: {"hairdressingTimeIds":hairdressingIds},
			dataType: "json",
...
//		alert("");
        var hairdressingTimeIds = $("#accountId_delete").val();
        $.ajax({
            type: "POST",
            url: "/hairdressing_time/batchDelete",
            data: {"hairdressingTimeIds":hairdressingTimeIds},
            dataType: "json",
            success: function(result){
                $.cookie.json = true;
                if(result.success){
		...
		init:function(){
    		...
    		$("#selectAll").change(function(){
    			if($(this).attr("checked")){
    				$("table#fromHairdressingTimeTable tbody input[type='checkbox']").uniform().each(function(index, item){
    		...
    		$("#createHairdressingTime").click(function(){
    			var url =  $("#setOptionUrl").val() + "index";
    			window.location.href=url;  
    		});
    		
    		$("#deleteHairdressingTimeBatch").click(function(){
    			var hairdressingIds='';
    			$("table#fromHairdressingTimeTable tbody input[type='checkbox']").uniform().each(function(index, item){
            ...
            $("a[name=deleteHairdressingTime]").click(function(){

                $("#accountId_delete").val($(this).attr("hairdressingTime_id"));

                var name=$($(this).parent().parent().find("td")[1]).text();
                name=name.replace(/(^\s*)|(\s*$)/g,'');
                confirm("删除美容时间","确认删除这些美容时间，开始时间：  "+ name +"?",deletePromotion);
            });

            $("a[name=editHairdressingTime]").click(function(){
                var url = $("#setOptionUrl").val() + "index" + "?" + "hairdressingTime_id=" + $(this).attr("hairdressingTime_id");
                window.location.href=url;
            })
