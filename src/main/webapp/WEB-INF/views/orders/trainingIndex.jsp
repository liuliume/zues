						...
						<div class="tabbable">
							<div class="tab-content no-space">
								<div class="form-body">
									...
									<div class="form-group">
										<label class="col-md-2 control-label">训练开始日期: <span
											class="required"> * </span>
										</label>
										<div class="col-md-3">
											<input type="text" class="form-control" id="startDate"
												...
												class="help-block display-hide" id="errorInfoStartDate">请选择训练开始日期!</span>
										</div>
									</div>

									<div class="form-group">
										<label class="col-md-2 control-label">训练结束日期: <span
											class="required"> * </span>
										</label>
										<div class="col-md-3">
											<input type="text" class="form-control" id="endDate"
												name="endDate" value="${orders.endDate}" readonly="readonly">
											<span class="help-block display-hide" id="errorInfoEndDate">请选择训练结束日期!</span>
										</div>
									</div>

									<div class="form-group">
										<label class="col-md-2 control-label">训练课程: <span
											class="required"> * </span>
										</label>
										<div class="col-md-10">
											<select
												...
												<c:forEach items="${allCourse }" var="item">
													<option value="${item.id}"
														<c:choose>
															<c:when test='${empty orders }'>
																<c:if test='${item.id eq -1}'>selected</c:if>
															...
															<c:otherwise>
																<c:if test='${item.id eq orders.courseId}'>selected</c:if>
															</c:otherwise>
														</c:choose>>
														${item.courseName }</option>
												</c:forEach>
											</select> <span class="help-block display-hide" id="errorInfoCourse">请选择课程!</span>
										</div>
									</div>
